...
class ModuleLoader:
    ...
    def __init__(
        self, bus, config: Dict[str, Any], modules_dirs: list[Path] | None = None
    ):
        self.bus = bus
        self.config = config
            ...
            for item in modules_dir.iterdir():
                ...
                try:
                    ...
                    if manager_class is None:
                        LOGGER.debug(
                            "Skipping %s: no ModuleBase subclass found", item.name
                        )
                        continue
                    ...
                    if manager_class.MODULE_NAME == "unnamed":
                        LOGGER.warning(
                            "Module %s has unnamed MODULE_NAME, using directory name",
                            item.name,
                        )
                    ...
                    if manager_class.MODULE_NAME in discovered:
                        LOGGER.debug(
                            "Overriding module %s from %s with version from %s",
                            manager_class.MODULE_NAME,
                            discovered[manager_class.MODULE_NAME],
                            modules_dir,
                        )
